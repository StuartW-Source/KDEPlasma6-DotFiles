...
;// ../../fragment/yt-wall-detection/dist/shared/messages.types.js
...
const messages_types_ytHavenToggleState = "yt-haven.toggle-state";
const ytHavenToggleStateUpdate = "yt-haven.toggle-state-update";
const ytHavenConflict = "yt-haven.conflict";
const messages_types_ytHavenRecordEvent = "yt-haven.record-event";
...
;// ../../fragment/yt-wall-detection/dist/content/shared.js
...
;// ../../fragment/yt-wall-detection/dist/content/player-controls/index.js
...
function getWatchingButton() {
    ...
    btn.style.fontSize = "109%";
    btn.style.cursor = "pointer";
    btn.style.display = "inline-block";
    btn.style.verticalAlign = "top";
    btn.style.margin = "0px 12px";
    ...
    btnText.textContent = browser_polyfill.i18n.getMessage("yt_haven_player_watching_with_label", manifest.short_name);
    btnText.style.borderBottom = "2px #e1002d solid";
    btnText.style.paddingBottom = "2px";
    btn.appendChild(btnText);
    return btn;
}
async function start() {
    const isEnabled = await isInHavenExperiment();
    if (!isEnabled || !window.location.hash.includes("yt-haven")) {
        return;
    }
    const controlsContainer = await waitForElement(".ytp-right-controls");
    const watchingBtn = getWatchingButton();
    watchingBtn.addEventListener("click", () => {
        const response = confirm(browser_polyfill.i18n.getMessage("yt_haven_player_watching_with_confirmation", manifest.short_name));
    ...
    controlsContainer.insertBefore(watchingBtn, controlsContainer.firstChild);
}
void start();
